...
export async function findProductByIdInBarbershop(barbershopId: string, productId: string, tx?: Prisma.TransactionClient) {
    const db = dbClient(tx);
    return db.products.findFirst({
        where: { id: productId, barbershop_id: barbershopId },
    });
}
...
export async function updateProductInBarbershop(
    ...
    const db = dbClient(tx);
    // Prisma não permite update com where composto sem unique -> então atualizamos por id,
    // mas SEMPRE antes garantimos que o registro é da barbearia (no service)
...
export async function deleteProductById(productId: string, tx?: Prisma.TransactionClient) {
    const db = dbClient(tx);
